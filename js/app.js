...
document.getElementById('btn-logout').onclick=()=>{ localStorage.removeItem(AUTH_KEY); location.href='login.html'; };
...
function drawLineChart(canvas, values){
  ...
  for(let i=1;i<=4;i++){ const y=pad+i*(H-pad*2)/5; ctx.beginPath(); ctx.moveTo(pad,y); ctx.lineTo(W-pad,y); ctx.stroke(); }
...
function renderDashboard(){
  ...
  const monthMoney=Array.from({length:12},(_,i)=>{
    const m=i+1; const rowsM=entriesOfMonth(state.year,m);
    return +rowsM.reduce((s,r)=>s+r.money,0).toFixed(2);
  });
  ...
  rows.forEach(r=>{
    ...
    el.innerHTML=`<div><strong>${r.date}</strong><div class="muted">1×:${r.h1||0} 1.5×:${r.h15||0} 2×:${r.h2||0} 3×:${r.h3||0}</div></div>
      <div style="display:flex;gap:6px;align-items:center"><span class="pill">${r.hours.toFixed(2)} ชม.</span><span class="pill money">${thb(r.money)}</span></div>`;
    list.appendChild(el);
  });
}
...
function renderReports(){
  ...
  el.innerHTML=`<div><strong>รวมเดือน ${fmtMonth(y,m)}</strong></div>
    <div style="display:flex;gap:6px;align-items:center"><span class="pill">${sumH.toFixed(2)} ชม.</span><span class="pill money">${thb(sumM)}</span></div>`;
  list.appendChild(el);
}
...
$('#btn-save').onclick=()=>{
  ...
  db.upsert(date, entry); renderAll();
};
$('#btn-delete').onclick=()=>{
...
$('#btn-export-csv').onclick=()=>{
  ...
  const csv=[header, ...rows.map(r=>[
    r.date,r.rate,r.h1||0,r.h15||0,r.h2||0,r.h3||0,r.hours.toFixed(2),r.money.toFixed(2),`"${(r.note||'').replace(/"/g,'""')}"`
  ].join(','))].join('\n');
  download(`ot_${y}-${pad(m)}.csv`, csv, 'text/csv');
};
$('#btn-export-json').onclick=()=> download('ot_backup.json', JSON.stringify(db.load(),null,2), 'application/json');
...
// Settings
$('#btn-save-defrate').onclick=()=>{
  const v=parseFloat($('#def-rate').value||'0'); const rate=v>0?v:0; db.setDefRate(rate);
  alert('บันทึกค่าดีฟอลต์แล้ว'); renderSettings();
};
$('#btn-salary-convert').onclick=()=>{
...
$('#btn-change-pin').onclick=()=>{
  const cur=$('#pin-current').value||''; const nw=$('#pin-new').value||'';
...
// init defaults
(function init(){
  // Default month label on first load
  renderAll();
  // PWA SW
